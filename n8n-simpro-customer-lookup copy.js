/**
 * n8n Code node: Look up Simpro Customer ID from client/company name
 *
 * SETUP (pick one):
 * 1) Companies on item: previous node sets json.companies = [ { ID, CompanyName }, ... ]
 * 2) Companies from another node: set GET_COMPANIES_NODE_NAME below to your node name
 * 3) Static list: set USE_STATIC_LIST = true and paste array into COMPANIES below
 *
 * INPUT: Items with client name in: json.client, json.customer, or json.company
 * OUTPUT: Same items + json.customerId, json.matchedCompanyName, and json.customerMatchError if no match
 */

const GET_COMPANIES_NODE_NAME = "Bill's Restaurants Ltd";  // e.g. 'Simpro Get Companies' - leave '' to use item.json.companies or COMPANIES
const USE_STATIC_LIST = false;       // true = use COMPANIES array below (paste your full list)



// do not delete this array, it is used to lookup the customer ID
const COMPANIES = [
    {
        "ID": 8,
...
function findCustomerId(clientName, companies) {
  if (!clientName || !companies || !Array.isArray(companies) || companies.length === 0) {
    return { id: null, name: null, error: 'No client name or companies list' };
  }
  const search = normalize(clientName);
  if (!search) return { id: null, name: null, error: 'Empty client name' };

  // 1) Exact match
  let match = companies.find(c => normalize(c.CompanyName) === search);
  if (match) return { id: match.ID, name: match.CompanyName, error: null };

  // 2) CompanyName contains search
  match = companies.find(c => normalize(c.CompanyName).includes(search));
  if (match) return { id: match.ID, name: match.CompanyName, error: null };

  // 3) Search contains CompanyName
  match = companies.find(c => search.includes(normalize(c.CompanyName)));
  if (match) return { id: match.ID, name: match.CompanyName, error: null };

  return { id: null, name: null, error: `No match for "${clientName}"` };
}
...
if (USE_STATIC_LIST) {
  companies = COMPANIES;
} else if (GET_COMPANIES_NODE_NAME) {
  const ref = $(GET_COMPANIES_NODE_NAME);
  const allItems = ref.all();
  ...
  if (Array.isArray(first)) {
    ...
    companies = allItems.map(i => i.json).filter(c => c && (c.ID != null && c.CompanyName != null));
  }
} else {
